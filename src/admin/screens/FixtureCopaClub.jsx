...
function PrePublicacion({ zonaRef, ligaId, grupos, clubes, categorias, gruposFixtureConf, setGruposFixtureConf, onPublicado }) {
  ...
  async function confirmarGrupo(grupoId, confData, fixtureBase) {
    const nuevoConf = { ...gruposFixtureConf, [grupoId]: { ...confData, generado: true } };
  ...
  return (
    <>
      ...
      <GrupoFixtureConfig
        ...
        onConfirmar={(confData, fixtureBase) => confirmarGrupo(grupoSel.id, confData, fixtureBase)}
...
function GrupoFixtureConfig({ grupo, clubes, conf, onConfirmar, onDesconfirmar, zonaRef, categorias, ligaId }) {
  ...
  function buildFixtureAuto() {
    ...
    rondas.forEach((ronda, ji) => {
      ...
      ronda.forEach(([lId, vId]) => {
        ...
        items.push({
          jornada, fecha: fechas[ji] || "", esLibre,
  ...
  function buildFixtureManual() {
    ...
    fechasManual.forEach((fd, ji) => {
      fd.partidos.forEach(p => {
        ...
        items.push({
          jornada: ji + 1, fecha: fd.fecha || "", esLibre: false,
          grupoId: grupo.id, grupoNombre: grupo.nombre,
  ...
  // ── Vista de grupo confirmado ────────────────────────────────────────────────
  if (conf.generado) {
    let totalPartidos, totalFechas;
    if (conf.totalPartidos != null) {
      totalPartidos = conf.totalPartidos;
      totalFechas   = conf.totalFechas ?? 0;
    } else {
      ...
      totalPartidos = confRondas.reduce((s, r) => s + r.filter(([l, v]) => l !== null && v !== null).length, 0);
      totalFechas   = confRondas.length;
    }
    return (
      <>
        <div style={{ background: "#f0fdf4", border: "1.5px solid #4ade80", borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#6b7280" }}>
              {totalPartidos} partido{totalPartidos !== 1 ? "s" : ""} · {totalFechas} fecha{totalFechas !== 1 ? "s" : ""}
              {conf.modo !== "manual" && conf.idaYVuelta ? " (ida y vuelta)" : ""}
            </div>
          </div>
  ...
  async function confirmar() {
    ...
    if (fixtureBase.filter(p => !p.esLibre).length === 0) { setError("No hay partidos para confirmar."); return; }

    const reales = fixtureBase.filter(p => !p.esLibre);
    const confData = modo === "automatico"
      ? { assigned, idaYVuelta, fechas: [...fechas], modo: "automatico", totalPartidos: reales.length, totalFechas: rondasVisibles.length }
      : { modo: "manual", fechasManual, totalPartidos: reales.length, totalFechas: fechasManual.length };

    setConfirmando(true);
    try {
      await onConfirmar(confData, fixtureBase);
    } catch (e) {
  ...
  const puedeConfirmar = totalActivos > 0 && (modo === "manual" || todosAsignados);
    ...
    <>
      ...
      {modo === "automatico" && (
        <>
          ...
          {todosAsignados && rondas.length > 0 && (
            <>
              <SeccionLabel>
                Fixture — {totalRealesAuto} partido{totalRealesAuto !== 1 ? "s" : ""} · {rondasVisibles.length} fecha{rondasVisibles.length !== 1 ? "s" : ""}
                ...
                return (
                  <Card key={ji}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "#f0fdf4", borderBottom: "1px solid #dcfce7" }}>
                      ...
                      <button onClick={() => setFechasBorradas(prev => new Set([...prev, ji]))} title="Eliminar esta fecha"
      ...
      {puedeConfirmar && (
        <button onClick={confirmar} disabled={confirmando}
          ...
          {confirmando ? "Confirmando..." : `✓ Confirmar ${grupo.nombre} · ${totalActivos} partido${totalActivos !== 1 ? "s" : ""}`}
        </button>
      )}
    </>
  );
...
// ── Fixture manual por grupo ───────────────────────────────────────────────────
function ManualFixtureEditorGrupo({ clubes, grupoNombre, fechasManual, setFechasManual, totalRealesManual }) {
  function addFecha() {
  ...
  return (
    <>
      <SeccionLabel>
        Fixture manual — {grupoNombre}{totalRealesManual > 0 ? ` · ${totalRealesManual} partido${totalRealesManual !== 1 ? "s" : ""}` : ""}
      </SeccionLabel>
...
function PostPublicacion({ zonaRef, zona, grupos, clubes, categorias, ligaId, onEditarFixture }) {
  ...
  async function exportarFixture() {
    ...
    try {
      await exportarFixtureExcel({ zonaRef, zona, categorias, clubes, grupos });
